  ...
  for( int i = 1; i < input.size() - 1; ++i )
  {
    for( int j = 1; j < input.front().size() - 1; ++j )
    {
      if( input[i][j] != '.' )
      {
        ...
        if( input[i][j] == '>' ) aux.dir = Vector2I( 1, 0 );
        if( input[i][j] == '<' ) aux.dir = Vector2I( -1, 0 );
        if( input[i][j] == '^' ) aux.dir = Vector2I( 0, -1 );
        if( input[i][j] == 'v' ) aux.dir = Vector2I( 0, 1 );
...
int reachGoal( const Vector2I& initPos, const Vector2I& endPos, std::vector<Blizzard>& blizzards, const BoundingBox& box )
{
  ...
  while( !currentPositions.contains( endPos ) )
  {
    ...
    {
      bli.pos += bli.dir;
      if( bli.pos.x == box.min.x ) bli.pos.x = box.max.x - 1;
      if( bli.pos.y == box.min.y ) bli.pos.y = box.max.y - 1;
      if( bli.pos.x == box.max.x ) bli.pos.x = box.min.x + 1;
      if( bli.pos.y == box.max.y ) bli.pos.y = box.min.y + 1;
