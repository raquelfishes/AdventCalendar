...
int getEvent()
{
	...
	while( !isValidYear )
	{
		std::cout << "Introduce el año del Advent Of Code que quieres ejecutar:";
		std::cin >> year;
		std::cout << std::endl;
		...
		{
			std::cout << "El año introducido no es valido, introduce un año entre 2015 y 2024" << std::endl;
		}
	}
	return year;
}
...
int main( int argc, char* argv[] )
{
	...
	// Crear el mapa de funciones automáticamente
	auto designs = createDesignMap<2023, 2024>();

	// Seleccionar el diseño según el año
	if (designs.find(year) != designs.end())
	{
		designs[year]();  // Llama a la función correspondiente
	}
	else
	{
		std::cout << "Año no soportado.\n";
	}
