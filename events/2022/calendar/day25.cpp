...
long long convertFromSNAFU( const std::string& s )
{
  ...
  for( int i = 0; i < s.length(); ++i )
  {
    ...
    char c = s[i];
    if( std::isdigit( c ) ) result += char2int(c) * powValue;
    else if( c == '-' ) result -= powValue;
    else if( c == '=' ) result -= ( powValue * 2 );
  }
  return result;
}
...
std::string convertToSNAFU( long long value )
{
  ...
  while( value )
  {
    ...
    if( b )
    {
      if( aux == 3 ) result = '=' + result;;
      if( aux == 4 ) result = '-' + result;
    }
    else
    {
