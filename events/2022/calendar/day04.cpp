...
struct ElfCouple
{
    ...
    bool fullOverlap()
    {
        if (initElf2 <= initElf1 && endElf2 >= endElf1)
            return true;
        if (initElf1 <= initElf2 && endElf1 >= endElf2)
            return true;
        return false;
    }
...
void day4Part1()
{
    std::string resourcePath = getResourcePath(2022, 4);
    std::vector<ElfCouple> input;
    readDocument<ElfCouple>(resourcePath, input);
    int count = 0;
    for (auto &elf : input)
    {
        if (elf.fullOverlap())
