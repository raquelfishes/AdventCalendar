...
struct BluePrint
{
    ...
    OreClayObsiGeo canBuildRobot(const OreClayObsiGeo &res) const
    {
        ...
        if (res >= oreRobotCost)
            std::get<0>(robot) = 1;
        if (res >= clayRobotCost)
            std::get<1>(robot) = 1;
        if (res >= obsiRobotCost)
            std::get<2>(robot) = 1;
        if (res >= geoRobotCost)
            std::get<3>(robot) = 1;
        return robot;
...
{
    // Is action helpful
    if (!isActionHelpful(action, bluePrint.maxCostRobots, robotsState))
        return;
    if (calcTheoreticalMax(time, resources, robotsState) <= maxGeodes)
        return;
...
void day19Part1()
{
    std::string resourcePath = getResourcePath(2022, 19);
    std::vector<BluePrint> input;
    readDocument<BluePrint>(resourcePath, input);
