...
struct Point
{
    ...
    Point(const int i, const int j, const int k) : x(i), y(j), z(k)
    {
    }

    Point(const int v) : x(v), y(v), z(v)
    {
    }
...
void day18Part1()
{
    std::string resourcePath = getResourcePath(2022, 18);
...
void day18Part2()
{
    std::string resourcePath = getResourcePath(2022, 18);
    std::vector<Point> input;
    readDocument<Point>(resourcePath, input);
