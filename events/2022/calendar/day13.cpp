...
result_enum correctOrderPackets( Packet p1, Packet p2 )
{
  ...
  {
    if ( i == p1.size() ) return e_correct_order;
    else if ( i == p2.size() ) return e_wrong_order;
    else
    {
      correct = std::visit( overloaded
                            {
                              []( const Packet& p1, const Packet& p2 ) { return correctOrderPackets( p1,p2 ); },
                              [] ( const Packet& p1, int p2 ) { return correctOrderPackets( p1,{p2} ); },
                              [] ( int p1, const Packet& p2 ) { return correctOrderPackets( {p1}, p2 ); },
                              [] ( int p1, int p2 )
                                  {
                                    if ( p1 < p2 ) { return e_correct_order; }
                                    else if ( p1 > p2 ) { return e_wrong_order; }
                                    else { return e_equal_order; }
                                  }
                            }, p1[i], p2[i] );
    }
  }
...
void day13Part1()
{
  ...
  readDocumentSeveralLines( DAY13_PATH, 2, input );
