...
void day3Part1()
{
    std::string resourcePath = getResourcePath(2022, 3);
    ...
    for (auto &line : input)
    {
        ...
        for (int i = 0; i < sizeC; ++i)
        {
            ...
            {
                int value;
                if (isCharLowerCase(line[sizeC + i]))
                    value = line[sizeC + i] - 'a';
                if (isCharUpperCase(line[sizeC + i]))
                    value = line[sizeC + i] - 'A' + 26;
...
void day3Part2()
{
    ...
    for (int i = 0; i < input.size(); i += 3)
    {
        ...
        int value;
        if (isCharLowerCase(intersection2[0]))
            value = intersection2[0] - 'a';
        if (isCharUpperCase(intersection2[0]))
            value = (intersection2[0] - 'A') + 26;
