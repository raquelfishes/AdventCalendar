...
void day22Part1()
{
    std::string resourcePath = getResourcePath(2022, 22);
    ...
    std::for_each(input.begin(), input.end(), [&maxX](const std::string &s)
                  { maxX = std::max(maxX, s.size()); });
    ...
    for (int i = 0; i < input.size(); ++i)
    {
        ...
        {
            if (j < input[i].size())
                grid.setValue(Coord(j, i), input[i][j]);
            else
                grid.setValue(Coord(j, i), ' ');
        }
    }
    ...
    {
        std::string count = "";
        while (std::isdigit(instrucions[instrIndex]))
        {
            count.push_back(instrucions[instrIndex++]);
        }
        ...
        if (instrucions[instrIndex] == 'R')
            moveIndex = (moveIndex + 1) % moveVector.size();
        else if (instrucions[instrIndex] == 'L')
            moveIndex = (moveIndex - 1) % moveVector.size();
        instrIndex++;
    }
