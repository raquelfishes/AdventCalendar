...
namespace
{
    ...
    struct MonkeyInfo
    {
        ...
        MonkeyInfo(const std::string &s)
        {
            ...
            name = values[1];
            if (isAllDigits(values[2]))
                value = std::stoi(values[2]);
            else
                instruction = Instruction(values[2]);
        }
...
void day21Part1()
{
    std::string resourcePath = getResourcePath(2022, 21);
    std::vector<MonkeyInfo> input;
    readDocument<MonkeyInfo>(resourcePath, input);

    std::map<std::string, MonkeyInfo> inputMap;
    std::for_each(input.begin(), input.end(), [&inputMap](const MonkeyInfo &m)
                  { inputMap[m.name] = m; });
