...
void mixList(std::list<Value> &listInput, const int listSize, const int roundCount)
{
    ...
    if (print)
    {
        std::for_each(listInput.begin(), listInput.end(), [](const Value &v)
                      { std::cout << v.val << " "; });
        std::cout << " " << std::endl;
    }
    ...
    for (int j = 0; j < roundCount; ++j)
    {
        ...
        for (int i = 0; i < listSize; ++i)
        {
            ...
            it = std::find_if(listInput.begin(), listInput.end(), [i](const Value &v)
                              { return v.id == i; });
            if (it->id == i)
            {
                Value aux = *it;
                ...
                for (int k = 0; k < steps; ++k)
                {
                    if (aux.val > 0)
                    {
                        if (newIt == listInput.end())
                            newIt = listInput.begin();
                        newIt++;
                    }
                    if (aux.val < 0)
                    {
                        if (newIt == listInput.begin())
                            newIt = listInput.end();
                        newIt--;
                    }
                }
                if (aux.val < 0 && newIt == listInput.begin())
                    newIt == listInput.end();
                if (aux.val > 0 && newIt == listInput.end())
                    newIt == listInput.begin();
...
{
    auto it0 = std::find_if(listInput.begin(), listInput.end(), [](const Value &v)
                            { return v.val == 0; });
...
{
    Index = 0;
    std::string resourcePath = getResourcePath(2022, 20);
    std::vector<Value> input;
    readDocument<Value>(resourcePath, input);
...
void day20Part2()
{
    ...
    std::for_each(input.begin(), input.end(), [&](Value &v)
                  { v.val *= decryptKey; });
