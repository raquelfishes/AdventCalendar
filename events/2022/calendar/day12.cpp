...
void dijstraCompute(GridI &grid, std::vector<Coord> initPositions)
{
    ...
    while (!toVisited.empty())
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (!grid.isValidCoord(next))
                continue;
            int nextId = grid.getIndex(next);
            if (grid.visited[nextId])
                continue;
...
void day12Part1()
{
    std::string resourcePath = getResourcePath(2022, 12);
