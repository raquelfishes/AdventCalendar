...
struct Monkey
{
    Monkey(std::vector<std::string> &input)
    {
        ...
        std::for_each(splittedText.begin(), splittedText.end(), [&](std::string &s)
...
void day11Part1()
{
    std::string resourcePath = getResourcePath(2022, 11);
    std::vector<Monkey> input;
    readDocumentSeveralLines(resourcePath, 6, input);
    ...
    std::sort(input.begin(), input.end(), [](Monkey &m1, Monkey &m2)
