...
{
    grid.sizeX = (int)str[0].size();
    grid.sizeY = (int)str.size();
...
float calculateH(const int x, const int y, const GridI &grid)
{
    ...
    return (float)std::sqrt(std::pow((grid.sizeX - 1) - x, 2) +
...
void aStarPathFinding(GridI &grid)
{
    ...
    int finalIndex = (int)grid.values.size() - 1;
...
void chitonPart1()
{
    std::string resourcePath = getResourcePath(2021, 15);

    std::vector<std::string> docuStr;
    readDocument<std::string>(resourcePath, docuStr);
